...
export default function Galaxy({
  ...
  useEffect(() => {
    ...
    if (!container || typeof window.WebGLRenderingContext === 'undefined') return undefined

    let renderer
    try {
      renderer = new Renderer({ alpha: transparent, premultipliedAlpha: false })
    } catch {
      container.dataset.galaxyFallback = 'true'
      return undefined
    }
    ...
    const update = (time) => {
      ...
      if (!disableAnimation) animationId = requestAnimationFrame(update)
    }

    const eventTarget = interactionTarget === 'parent' ? container.parentElement : container
    ...
    const handleMouseLeave = () => { targetMouseActive.current = 0 }

    if (mouseInteraction && eventTarget) {
  ...
  }, [autoCenterRepulsion, density, disableAnimation, focal, glowIntensity, hueShift, interactionTarget, mouseInteraction, mouseRepulsion, repulsionStrength, rotation, rotationSpeed, saturation, speed, starSpeed, transparent, twinkleIntensity])

  return <div ref={containerRef} className={`galaxy-container ${className}`} data-interaction-target={interactionTarget} {...rest} />
}
